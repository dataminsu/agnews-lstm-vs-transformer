...
# The shared modules (data_pipeline.py, models.py, text_utils.py) live one level
# up in code/. Add that dir to sys.path so this script imports them no matter the
# current working directory or PYTHONPATH.
import sys
...
def main():
    ...
    ap.add_argument("--order-condition", type=str,   default="original",
                    choices=["original", "local_shuffle", "full_shuffle"],
                    help="Word-order perturbation ablation. Applied to all splits, "
                         "keyed on data_seed (fixed across seeds).")
    ap.add_argument("--perturb-window",  type=int,   default=5,
                    help="Window size for --order-condition local_shuffle.")
    ...
    ap.add_argument("--save-plots",      action="store_true",
                    help="Save loss_curves.png and confusion_matrix.png")
    ...
    cfg = DataConfig(
        ...
        order_condition=args.order_condition,
        perturb_window=args.perturb_window,
    )
    print(f"config: {cfg}")
